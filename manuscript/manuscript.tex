...
the rod axis. Here $k_i$ is not a fixed sign but a dichotomous noise: $k_i=\pm1$ with
equal probability, switching at rate $\frev$,
\begin{equation}
    \label{eq:k-dichotomous}
    \la k_i(t)k_j(t')\ra = \delta_{ij}\,\ee^{-2\frev|t-t'|}.
\end{equation}
This is the periodic reversal of \textit{Myxococcus xanthus} gliding motility. On
timescales long compared with $\frev^{-1}$ we approximate $k_i$ by Gaussian white noise,
...
field. The third term is the active force density
\begin{align}
...
\end{align}
the divergence of the active nematic stress $-\zeta\Q$. Positive $\zeta$ is extensile.
The coefficient comes entirely from self-propulsion and reversal, and grows with the
distance $v_0/\frev$ a cell glides between reversals, so a colony that reverses more
slowly is more active.
...
$\int\!\d\theta\,\partial_\theta(\cdot)=0$. The noise sits inside the divergence, so it
conserves the number of cells exactly and not only on average.
...
\end{align}
Every term on the right is a force density. The monolayer is overdamped, so forces set
velocities rather than accelerations. The osmotic force $-B\nabla\rho$ is the largest and
keeps the monolayer nearly uniform. The elastic force $\Q\!:\!\nabla\H$ redistributes
cells to relieve orientational stress. Only $\fa$ does not derive from $F$: the passive
forces vanish at a minimum of $F$ while $\fa$ does not, and this is the whole of the
activity in the density channel. Since $\fa\propto\nabla\!\cdot\!\Q$, it vanishes in a
uniform texture and is largest where the texture bends fastest, at a defect
...
the system: the angular averages $\la\hat\Q\hat\Q\ra_\varrho$ and
$\la\hat\Q'\hat\Q'\ra_\varrho$ appear, and $\rho$ and $\Q$ do not determine them. We use
...
higher-order terms, and the limitations of \cref{eq:closure}, which affect only the
numerical solutions.
...
$\tfrac4{\xir}\Ha$ relaxes $\Q$ toward a minimiser of the effective free energy, and
...
which we cannot treat analytically and which are expensive on a grid. The two terms must
...
solves. The activity enters them twice under one coefficient: as the force $\fa$ and as
...
deterministic steady state $\rho_\ss(\r)$, $\Q_\ss(\r)$ of the noiseless equations
...
density. \Cref{app:parameters-choice} gives their values and \cref{app:comoving} the
...
Pad\'e approximation \eqref{eq:pade-approximation}: the director rotates by at most one
...
short-length cutoff of its own, so we impose the physical one, the width of a bacterium,
\begin{equation}
    \label{eq:cutoff}
    \elln = 0.5~\micron.
\end{equation}
...
\end{equation}
\Cref{eq:total-gauss} is the central result of \cref{sec:defect}. The pedestal is
uniform: it is the price of resolving conserved shot noise at scale $\elln$, and it
carries no information about the defect. The defect enters only through the bracket, as
...
\cref{sec:defect-steady} nor the drift speed $u$ plays any part. The defect is visible in
