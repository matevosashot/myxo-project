...
controlled by one coefficient, which we define in \cref{eq:active-force} once the fields it acts
...
The joint density $\varrho_t$ carries far more information than we need. What the rest
...
equations for these two fields. Note that the polar moment
...
The last term is the noise, and it is a fluctuating cell \emph{flux} rather than a
force. It descends from the reversal term $v_0\p_ik_i$ of
\cref{eq:langevin-position}, which is a velocity, not a force, which is why it sits
...
$\Q$, both carrying the same $\zeta$ as the active force \cref{eq:active-force}. The
...
cells are conserved fluctuation by fluctuation rather than only on average, which is
what fixes the wavevector dependence of the density fluctuations in
...
\begin{align}
  \label{eq:velocity}
  \v = \frac{1}{\xi_0}\bigl(-B\nabla\rho + \Q\!:\!\nabla\H + \fa\bigr) + \bEta_{\rho}.
\end{align}
...
$\theta$ does not close it. Where the density moment integrated exactly, term by term,
the nematic moment leaves behind angular averages
...
closure is therefore unavoidable, and we take the lowest-order maximum-entropy one,
\begin{align}
    \label{eq:closure}
...
$O(S^2)$ in the nematic order $S=|\Q|/\rho$ and is the leading term of a systematic
expansion. \Cref{app:closure} gives the exact moments, the expansion, and the range of
$S$ over which the noise covariance this closure implies stays positive semi-definite,
a limitation that bites on the numerical solver of \cref{app:lyapunov} rather than on
the analytic line of \cref{sec:defect-fluct}. With the closure in place we obtain
\begin{align}
  \label{eq:model-Q}
  \partial_t \Q ={}& -\frac{1}{\xi_0} \nabla\!\cdot\!\Bigl(\!\!-\!\Q B \nabla\rho
  + \nabla \H\Bigr) + \frac4{\xir} \Ha + \f^Q(\r,t).
\end{align}
...
solution in this paper is built from: the core size $\elld=\sqrt{K'/(-a')}$ and the
...
$\tfrac4{\xir}\Ha$ relaxes $\Q$ toward a minimiser of the renormalized free energy, and
...
deform it weakly. Solving for that translation is the business of
\cref{sec:defect-steady}.
...
from the bare coefficients; \cref{app:parameters-choice} gives their values, and
...
The density therefore departs from uniformity only at order $\zeta/B$, below $10^{-4}$
at the parameters used here: the monolayer is stiff, and it answers the active pumping
with a small density gradient rather than with a flow.
...
pushes the cells, and in a nearly incompressible layer that push is spent almost
entirely on building a density gradient. Setting the flux in \cref{simplified-noiseless}
to zero gives $B\nabla\rho\simeq\fa$: the osmotic force cancels the active one pointwise,
and no net cell current survives. What the density gradient does not cancel is its own
effect on the texture. The nematic flux in \cref{eq:model-Q} is
$\bs J_\Q = -\Q B\nabla\rho/\xi_0 \simeq -\Q\fa/\xi_0$, so $\Q$ is transported with
velocity $-B\nabla\rho/\xi_0$, down the density gradient and therefore against $\fa$.
The core travels with the texture.

Two consequences follow. The drift speed is independent of $B$, since the dipole
amplitude scales as $1/B$ and the transport coefficient as $B$; it is set by
$\zeta/\xi_0$ alone, survives the incompressible limit, and vanishes only with the
activity itself, which the solver confirms by returning $u\to0$ as $\zeta\to0$
(\cref{app:comoving-verification}). And the effect is specific to $+1/2$: a $-1/2$
defect is three-fold symmetric, generates no net active force, and neither drifts nor
builds a dipole.

For the $+1/2$ texture \cref{eq:pade-approximation} gives
\begin{align}
...
with $u_y$ at the noise level of the solve as the mirror symmetry requires.
...
\begin{align}
    \partial_t\delta\rho ={}& -\nabla\!\cdot\!\Bigl[
    \frac1{\xi_0}\bigl(-B\nabla\delta\rho + \delta\fa\bigr) + \bEta_\rho(\r,t)\Bigr],
...
stochastic field theory on the spatially varying background $(\rho_\ss,\Q_\ss)$, and its
equal-time covariance solves a Lyapunov equation, which \cref{app:lyapunov} sets up and
solves numerically with $\delta\Q$ retained.

In the main text we present the analytically tractable case, in which only the density
fluctuates and $\delta\Q$ is set to zero. This is the appropriate limit when
orientational relaxation is fast enough that the nematic field stays slaved to
$\Q_\ss$, and it has one feature that makes the whole calculation work: with
$\delta\fa=0$ the defect enters \cref{eq:linearized-rho} in exactly one place, through
the $\Q_\ss$ term in the noise autocorrelation \cref{eq:eta-rho-autocorrelation}. The
texture is felt by the fluctuations and not by the mean.
...
$\la\delta\rho(\r,t)\delta\rho(\r',t)\ra_\ss$ then diverges as $\r'\to\r$. The
divergence is one of short length scales, and the model has a shortest length of its
own: no cell can generate, and no measurement can resolve, a density fluctuation
sharper than a bacterium. We accordingly cut the theory off at
\begin{equation}
    \label{eq:cutoff}
    \elln = 0.5~\micron,
\end{equation}
the width of an \textit{M.\ xanthus} cell, and carry $\elln$ explicitly through
everything that follows rather than tuning it away
(\cref{app:lyapunov-finite-corr}).
...
\end{align}
the cutoff entering as an exponential suppression of wavevectors beyond $1/\elln$.

Consider first the isotropic case $\Q_\ss=0$. Inverting the transform over space and
time gives
\begin{align}
...
\end{align}
writing $\sigmarho(\r)\equiv\Var[\rho(\r)]$ for the rest of the paper.
...
defect; both are properties of a uniform monolayer, and $\tau_c$ is what will set the
attempt frequency in \cref{sec:nucleation-results}.
...
merely down-weighted but invisible. In Fourier space the same kernel is the exponential
integral $\hat\Phi_{\elln}(p)=\tfrac12E_1(p^2\elln^2/8)$, which crosses over from
$\ln(1/p\elln)$ to $4\ee^{-p^2\elln^2/8}/p^2\elln^2$ at $p\elln\simeq1$.

Note what does \emph{not} appear in \cref{eq:total-gauss}. The steady density
$\rho_\ss$ has dropped out beyond its far-field value, so the fore-aft dipole of
...
\begin{figure}[tbp]
  ...
  \caption{Equal-point density variance around a $+1/2$ defect, normalized by the
    isotropic pedestal of \cref{eq:total-g,auss}, along and across the defect axis.
