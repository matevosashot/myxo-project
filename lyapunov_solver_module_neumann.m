...
(* psi = (d\[Rho], dQ1, dQ2)^T on a uniform cell-centred grid with NO-FLUX walls;
   ...
   ========================== NEUMANN VARIANT =========================
   This is lyapunov_solver_module_R1.m with the boundary condition changed from
   the RESERVOIR case to the SEALED case.  dean.tex appendix sec-ou, subsection
   ...
   Entry point is SolveFluctuationsLyapunovNeumann and runTag carries a "_Neu"
   suffix, so outputs cannot be confused with either earlier scheme.  Do NOT load
   this module together with lyapunov_solver_module.m or _R1.m in the same
   kernel: the private helpers are shared and the last one loaded wins.
   ====================================================================
...
Clear[SolveFluctuationsLyapunovNeumann, visualizeFluctuations,
      exportData, MergeParams, ScriptParamOverrides];


SolveFluctuationsLyapunovNeumann::usage =
  "SolveFluctuationsLyapunovNeumann[lyapunovParams, modelParams, steadyState] solves " <>
...
SolveFluctuationsLyapunovNeumann::nullmode =
...
(* "_Neu" suffix: the reservoir and sealed schemes must never collide. *)
...
consCheck[lbl_, val_, scale_] := (
  ...
  If[!(Abs[val] <= 1.*^-10 scale),
     Message[SolveFluctuationsLyapunovNeumann::notcons, lbl, val, scale];
...
SolveFluctuationsLyapunovNeumann[
...
Module[
  ...
  If[!IntegerQ[nInt] || !IntegerQ[fboxV] || nInt <= 1 || fboxV <= 0,
    Message[SolveFluctuationsLyapunovNeumann::badgrid, {fboxV, nInt}]; Return[$Failed]];
  If[!AllTrue[{"rho", "Q1", "Q2"}, KeyExistsQ[steadyState, #] &],
    Message[SolveFluctuationsLyapunovNeumann::badss, Keys[steadyState]]; Return[$Failed]];
  ...
  If[kapResid > 1.*^-8, Message[SolveFluctuationsLyapunovNeumann::kappa, kapResid]];
  ...
  scaleQ = Max @ Abs @ Qtilde["NonzeroValues"];
  ...
  If[Min@minEigLoc < -1.*^-10 scaleLoc,
     Message[SolveFluctuationsLyapunovNeumann::notpsd,
  ...
  If[!(nullRel < 1.*^-8 && lam2Rel > 1.*^-6 && nullOverlap > 0.99),
     Message[SolveFluctuationsLyapunovNeumann::nullmode, nullRel, lam2Rel, nullOverlap]];
  ...
  If[maxReLam >= 0, Message[SolveFluctuationsLyapunovNeumann::unstable, maxReLam]];
  ...
  If[residRef > resid,
     Message[SolveFluctuationsLyapunovNeumann::norefine, resid, residRef]];
