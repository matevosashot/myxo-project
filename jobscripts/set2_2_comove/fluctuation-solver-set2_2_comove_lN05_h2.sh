...
# set2_2_comove, lNoise = 0.5 -- H2 tier (NEW; no lNoise = 1.73 counterpart).
# Companions: the other four tiers of
# fluctuation-solver-set2_2_comove_lN05_{lo,mid,hi,h2,xl}.sh.
#
# Nint = 140, 141 -- n = 58800 / 59643.  The measured law puts peak RSS at
# ~468-482 GiB, so 900G carries ~1.9x the predicted peak, the same headroom
# ratio _hi and _xl already run with.  The n^3 scaling puts the eigensolve at
# ~8.4 h and the pipeline at ~13 h, so 1-12:00:00 is ~2.8x the estimate --
# deliberately generous, because that estimate is an extrapolation from
# n = 11163, not a measurement anywhere near this size.  Covariance ~26 GiB
# each, ~260 GiB for this tier.
#
# This tier exists to sit between _hi (n = 50700) and _xl (n = 67500), which is
# a factor 1.33 in n and 2.4 in eigensolve cost -- too wide a gap to read a
# convergence trend across when the dQ sector is only trustworthy at the top.
#
# 10 tasks = 2 grids x 5 boxes (box = fbox = 10, 15, 25, 30, 40).
# ----------------------------------------------------------------------------
# WHAT THIS FAMILY IS
# ----------------------------------------------------------------------------
...
#SBATCH --array=0-9%2
#SBATCH --job-name="s22c05_h2"

#SBATCH --mem=900G
#SBATCH --partition=long
#SBATCH --time=1-12:00:00 # expected maximum runtime of job
