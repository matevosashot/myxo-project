...
# set2_2_comove, lNoise = 0.5 -- LO tier.  Companions: the other four tiers of
...
# growing with the box) is a noticeable share of the wall time.  Covariance
# files are ~0.5 GiB each.
#
# 8 tasks = 2 grids x 4 boxes.  THIS IS THE ONE TIER THAT IS NOT 2 x 5: box 40
# is dropped because Nint = 50, 51 sits below the lNoise = 0.5 calibration
# floor of 63 there.  See the floor table below.
#
# THIS TIER IS THE CONVERGENCE CURVE, NOT A MEASUREMENT.  ell_d = 0.8 against
# h = 2 box/(Nint+1) gives ell_d/h = 2.04 at box 10 and 1.36 at box 15, down to
# 0.68 at box 30, so away from the smallest boxes the defect core is around one
# cell and maxReLambda is expected positive.  Where it is positive the solution
# is not a covariance and Sigma_Q1/Q2 come back NEGATIVE against a perfectly
# healthy Sigma_rho.  Take Sigma_rho from these tasks; take the dQ sector from
# _hi, _h2 and _xl.
# ----------------------------------------------------------------------------
# WHAT THIS FAMILY IS
# ----------------------------------------------------------------------------
