...
# set2_2 BOX SCAN -- one (fbox, Nint) pair per array task, lNoise = Sqrt[3] throughout.
#
# Unlike the _box40 scripts, which fix the box and sweep the grid, this one varies
# BOTH so that box size and mesh spacing can be separated.  Reservoir wall
# (Dirichlet), consistent stencils (R == 1) plus the Gaussian UV cutoff; runs
# jobscripts/set2_2/fluctuations.wls -> lyapunov_solver_module_cutoff.m.  This is
# the Dirichlet partner of fluctuation-solver-set3_2_boxscan.sh, same eight rows.
#
...
# while tasks 6-7 take most of a day.
#
# h = 2 fbox/(Nint+1) for this node-centred scheme -- note the +1, which is what
# makes these numbers differ slightly from set3_2's cell-centred h = 2 fbox/Nint.
# lNoise is fixed at Sqrt[3] = 1.7321, so l/h decides how well the Gaussian cutoff
# is resolved; the module's accuracy rule is +(1/4)(h/l)^2, i.e. 1% at l = 5h (see
# the _box40.sh header for the derivation):
#
#   task  fbox  Nint     h       l/h    kernel err   n = 3 Nint^2   peak RSS
#   ----  ----  ----  -------  ------  -----------  -------------  ---------
#     0     60   120   0.9917   1.746      8.2%         43200        253 GiB
#     1    120   120   1.9835   0.873     32.8%         43200        253 GiB
#     2     60   121   0.9836   1.761      8.1%         43923        261 GiB
#     3    120   121   1.9672   0.880     32.2%         43923        261 GiB
#     4     40   140   0.5674   3.053      2.7%         58800        468 GiB
#     5     40   141   0.5634   3.074      2.6%         59643        482 GiB
#     6     75   150   0.9934   1.744      8.2%         67500        617 GiB
#     7     75   151   0.9868   1.755      8.1%         68403        634 GiB
#
# (fbox 40, Nint 160) and (fbox 40, Nint 161) are NOT here on purpose: they are
# already tasks iN=2,iL=1 and iN=3,iL=1 of fluctuation-solver-set2_2_box40_hi.sh,
...
# node-centred scheme h = 2 fbox/(Nint+1), so ODD Nint puts the defect core
# (S = 0) exactly on a node and EVEN Nint straddles it -- the same reason the
...
#   * Within this file: tasks 0 and 6 agree in h to 0.17%, so boxes 60 and 75
#     differ by box size alone, with essentially no discretisation confound; same
#     for their odd partners 2 and 7.
#   * Against the sealed partner: h here is 2 fbox/(Nint+1), while set3_2's
#     cell-centred h is 2 fbox/Nint, so an EVEN Nint here matches ODD Nint+1 there
#     at the same box, exactly -- (60,120)<->(60,121), (75,150)<->(75,151),
#     (120,120)<->(120,121), (40,140)<->(40,141).
#     fluctuation-solver-set3_2_boxscan.sh carries the identical row table in the
#     identical order, so the same task id is the same (fbox, Nint) in both files.
#
# READ THIS ABOUT TASKS 1 AND 3.  At l/h ~ 0.88 the Gaussian is narrower than one
# cell: it is essentially invisible to the grid, and the run reverts to the
# grid-limited no-cutoff (set2_1) answer.  Those rows measure the mesh, not the
...
# ElementMesh, which extrapolates.  Because fbox varies per row, box is set FROM
# the row, never hard-coded.
#
...
#
# NO OUTPUT COLLISIONS.  runTag is "box<fbox>_N<Nint>_lN<value>".  Boxes 60, 75 and
# 120 appear in no other sweep, and the box-40 rows use Nint = 140/141, which the
# _box40 scripts (20..131 and 150..161) do not cover.
...
# Identical table, identical order, in fluctuation-solver-set3_2_boxscan.sh.
...
# box tracks fbox -- see the header.
box=$fbox

# plotBox stays 15 across all rows, as in the other set2_2 scripts: with a
# reservoir wall the defect core is the subject, so a fixed crop keeps the figures
# comparable between rows even though the boxes differ.  The data files hold the
# full grid regardless.
plotBox=15
