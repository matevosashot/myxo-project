...
DATA_PATH = Path("/data/biophys/ashmat/data/myxo-project/shape_change/")
...
ratios = np.linspace(1, 7/0.5,41)
ratio = ratios[TASK_ID]
lM = np.sqrt(7 * ratio / 2)
lm = np.sqrt(7 / (2 * ratio))

os.makedirs(DATA_PATH, exist_ok=True)

print(f"Running for ratio {ratio} and lM {lM} and lm {lm}")
