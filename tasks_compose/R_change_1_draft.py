...
solver = FourierSolver(n=201, L=60, model_params=model_params,
                        dtype="float64", verbose=True, drop_phi=True, gcalc_Q_cls=GcalcForQ)
solver.precompute()

solver.solve_P_contribution__symmetry_optimized()
exit(0)

out = solver.save_results_h5(SCRATCH_PATH, save_P_diag=False, save_Q_diag=True)
exit(0)
print(f"Saved {out}")
